...
%--- This program calculates stock prices for Double-Barrier Stock Options
%via the CRR-Binomial Model ---
...
upper_barrier = 120;          %option barriers
lower_barrier = 90;
...
hold on;
plot([0, time],[upper_barrier, upper_barrier], 'r');
plot([0, time],[lower_barrier, lower_barrier], 'r');
hold on;
xlabel('Time');
ylabel('Stock Price');
hold on
...
for(i= 1:periods+1)
    if(PriceTree(i,j) >= upper_barrier ||  PriceTree(i,j) <= lower_barrier)        %%check if barrier was crossed
...
for j = periods:-1:1
    ctime = ctime - h;
    for(i = 1:j)
        if(PriceTree(i,j) >= upper_barrier || PriceTree(i,j) <= lower_barrier)        %check barrier
            ValueTree(i,j) = 0;
        else
